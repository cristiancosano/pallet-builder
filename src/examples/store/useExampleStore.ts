...
export interface ExampleActions {
  ...
  movePalletFromWarehouseToTruck: (
    warehousePlacementId: string,
    truckPosition: Position3D,
  ) => void;
...
function createDefaultBoxTemplates(): BoxTemplate[] {
  return [
    ...
    },
    {
      id: uid("tpl"),
      name: "Caja frágil",
      box: BoxFactory.fragile({ width: 250, height: 200, depth: 350 }, 10, {
        type: "Fragile",
        weight: 5,
        color: "#e8a838",
      }),
...
export const useExampleStore = create<ExampleState & ExampleActions>(
  (set, _) => ({
    ...
    selectedStrategy: "material-grouping",
    ...
    updateBoxTemplate: (id, updates) =>
      set((s) => ({
        boxTemplates: s.boxTemplates.map((t) =>
          t.id === id ? { ...t, ...updates } : t,
        ),
      })),
    ...
    removePallet: (id) =>
      set((s) => {
        ...
        return {
          ...
          warehouse: {
            ...s.warehouse,
            pallets: s.warehouse.pallets.filter(
              (pp) => pp.stackedPallet.id !== id,
            ),
          },
          truck: {
            ...s.truck,
        ...
        warehouse: {
          ...
          pallets: s.warehouse.pallets.map((pp) =>
            pp.id === placementId
              ? {
                  ...pp,
                  yRotation: ((pp.yRotation + 90) % 360) as 0 | 90 | 180 | 270,
                }
              : pp,
          ),
        },
    ...
    movePalletFromWarehouseToTruck: (warehousePlacementId, truckPosition) =>
      set((s) => {
        const whPallet = s.warehouse.pallets.find(
          (pp) => pp.id === warehousePlacementId,
        );
    ...
    setSelectedPalletPlacementId: (id) =>
      set({ selectedPalletPlacementId: id }),
